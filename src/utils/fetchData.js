...
async function reqData(url, api_url, res, cb) {
  try {
    ...
    console.error(
      `Error occurred during request: ${error}, type of cd ${typeof cb}`
    );

    try {
      console.log(`Retrying request with backup API: ${process.env.API1}`);
      const response = await axios.get(
        `https://${process.env.API1}/api?data=${url}`
      );
      ...
      console.error(
        `Failed to retrieve data from both APIs for URL: ${url}, Error: ${reError}`
      );
